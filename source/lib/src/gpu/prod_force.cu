  ...
  if (nloc != nall) {
    /*
      ...
      - To avoid the atomicAdd we treat each atom one by one and follow these
         ...
         (data are on this GPU), (ii) if so calculate the forces and accumulate
         the results locally (iii) when done apply a final reduction and store
         the results back in global memory
     */
...
void prod_force_a_r_gpu(FPTYPE* force,
  ...
  dim3 block_grid(nall, sqrt_nframes + 1, sqrt_nframes + 1);
