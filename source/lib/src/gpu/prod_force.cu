...
template <typename FPTYPE, bool use_deterministic_algo_ = false, bool radial_only_ = true>
    ...
    const int nnei)  // number fo neighbors
{
  ...
  */
  ...
  if (!use_deterministic_algo_) {
    for (int idx = threadIdx.x; idx < nnei; idx += blockDim.x) {
      ...
      if (j_atom >= 0) {
        ...
        force_tmp[1] = 0.;
        force_tmp[2] = 0.;
        calculate_forces<FPTYPE, radial_only_>(force_tmp,
                                               frame_net_deriv_,
                                               frame_in_deriv_,
                                               idx + nnei * atom_id);
  ...
  if (nloc != nall) {
    ...
    for (int neighbor_id = threadIdx.x;
         neighbor_id < nnei;
...
void prod_force_a_r_gpu(FPTYPE* force,
  ...
  if (use_deterministic_algorithm_) {
    force_deriv_wrt_neighbors<FPTYPE, true, radial_only_>
        <<<block_grid, thread_grid, 3 * 64 * sizeof(FPTYPE)>>>(force, net_deriv, in_deriv, nlist, nframes,
                                      nloc, nall, nnei);
  } else {
