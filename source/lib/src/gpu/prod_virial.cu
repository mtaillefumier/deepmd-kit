...
template <typename FPTYPE,
          bool radial_only_,
          bool use_deterministic_algorithm_ = false,
          int shared_mem_block_size_>
__global__ void virial_deriv_wrt_neighbors(FPTYPE* __restrict__ virial,
  ...
  if (use_deterministic_algorithm_) {
    ...
    /*
        ...
        over multiple thread blocks. The neighbor list does not allow that
       easily in the multi-gpu case that's why we have this complicated and
       expensive search. It can be improved at the expense of more memory.
    */
    ...
    /*
     * Original non-deterministic version of the function. It was
     * rewritten for clarity reason
     */
    ...
    for (int neighbor_id = threadIdx.x; neighbor_id < nnei;
      ...
      memset(virial_tmp, 0, sizeof(FPTYPE) * 9);
      calculate_virial<FPTYPE, radial_only_>(virial_tmp, frame_net_deriv_,
                                             frame_in_deriv_, frame_rij_,
                                             atom_id * nnei + neighbor_id);
      for (int deg = 0; deg < 9; deg++) {
        atomicAdd(atom_virial + 9 * atom_j + deg, virial_tmp[deg]);
      }
    }
  }
}
...
void prod_virial_a_r_gpu(FPTYPE* virial,
  ...
  if (use_deterministic_algorithm_) {
    virial_deriv_wrt_neighbors<FPTYPE, radial_only_, true, 64>
        <<<block_grid, thread_grid, 64 * sizeof(FPTYPE)>>>(
            virial, atom_virial, net_deriv, in_deriv, rij, nlist, nloc, nall,
            nnei);
  } else {
    block_grid.x = nloc;
    virial_deriv_wrt_neighbors<FPTYPE, radial_only_, false, 64>
        <<<block_grid, thread_grid, 64 * sizeof(FPTYPE)>>>(
            virial, atom_virial, net_deriv, in_deriv, rij, nlist, nloc, nall,
            nnei);
  }
